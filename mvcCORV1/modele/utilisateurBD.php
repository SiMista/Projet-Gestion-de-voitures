<?php
	...
	function verif_bd($email,$num,&$profil) {
		require('modele/connectBD.php'); //$pdo est défini dans ce fichier
		$sql="SELECT * FROM `utilisateur` WHERE email=:email AND num=:num";
		try {
	...
	function inscription($nom,$prenom,$email,$num){
		require('modele/connectBD.php'); //$pdo est défini dans ce fichier
		$sql =('INSERT INTO `utilisateur` (`prenom`, `nom`, `num`, `email`) VALUES (:prenom, :nom, :num, :email);');
			try {
			$commande = $pdo->prepare($sql);
